...
/* ━━━ ADMIN ━━━ */
function Admin({ sales, expenses, setExpenses, cambios, setCambios, tasa }) {
  ...
  PAY_METHODS.forEach(m => {
    ...
    byMethod[m] = { ing: ing + cIn, egr: egr + cOut, disp: ing - egr + cIn - cOut, ingBs: ingBs + cInBs, egrBs: egrBs + cOutBs, dispBs: ingBs - egrBs + cInBs - cOutBs };
  });
  ...
  byMethod["Cuenta Bancaria"] = { ing: bancoIng, egr: bancoEgr, disp: bancoIng - bancoEgr, ingBs: bancoIngBs, egrBs: bancoEgrBs, dispBs: bancoIngBs - bancoEgrBs };

  const chartData = dates.map(d => ({ l: d.split("-")[2], v: byDate[d].reduce((a, s) => a + s.pago, 0) }));
  const chartMax = Math.max(...chartData.map(d => d.v), 1);
  const tabs = [{ id: "resumen", em: "📊", label: "Resumen" }, { id: "dinero", em: "🏦", label: "Dinero" }, { id: "detalle", em: "📅", label: "Detalle" }, { id: "gastos", em: "📋", label: "Gastos" }, { id: "cambios", em: "💱", label: "Cambios" }, { id: "reportes", em: "📄", label: "Reportes" }];
  ...
  return (
    <div>
      ...
      {tab === "cambios" && <>
        <h3 style={{ margin: "0 0 12px 0", fontSize: 15, color: "#c0c0d8", fontWeight: 700 }}>Historial de Cambios ({cambios.length})</h3>
        {cambios.length === 0 ? (
...
export default function App() {
  ...
  const [tasa, setTasa] = useState(86.50);
  const [ready, setReady] = useState(false);
  const [tasaConfirmed, setTasaConfirmed] = useState(false);
  ...
  // Load from Firestore on mount (or skip if no db)
  useEffect(() => {
    setReady(true);
    ...
    const load = async () => {
      try {
        const [sSnap, eSnap, cSnap] = await Promise.all([
          ...
          getDocs(collection(db, "cambios")),
        ]);
        const s = sSnap.docs.map(d => d.data());
        const e = eSnap.docs.map(d => d.data());
        const c = cSnap.docs.map(d => d.data());
        setSales(s); setExpenses(e); setCambios(c);
        prevSales.current = s; prevExpenses.current = e; prevCambios.current = c;
        setDbStatus("ok");
      } catch (err) { console.error("Firebase load error:", err); setDbStatus("error"); }
      setDbLoaded(true);
    };
    load();
  }, []);
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#080818", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif", padding: 20, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ opacity: ready ? 1 : 0, transform: ready ? "translateY(0)" : "translateY(30px)", transition: "all 1s cubic-bezier(0.16, 1, 0.3, 1)", textAlign: "center", position: "relative" }}>
          <div style={{ width: 64, height: 64, borderRadius: 16, margin: "0 auto 28px", background: "linear-gradient(135deg, #4338ca, #6366f1)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 50px rgba(99,102,241,0.25)" }}>
            <span style={{ fontSize: 28, fontWeight: 800, color: "#fff", fontFamily: "'JetBrains Mono', monospace" }}>V$</span>
          </div>
          <h1 style={{ margin: "0 0 4px 0", fontSize: 26, fontWeight: 800, color: "#e2e2f0", letterSpacing: -1 }}>Sistema de Ventas</h1>
          <p style={{ margin: "0 0 8px 0", fontSize: 13, color: "#4a4a6a" }}>Notas de venta · Cuadre de caja · Doble moneda</p>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 14px", borderRadius: 20, background: "#1a1a0e", border: "1px solid #2d2510", marginBottom: 36 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#eab308", animation: "pulse 2s infinite" }} />
            <span style={{ fontSize: 12, color: "#a38a2d", fontWeight: 600 }}>Tasa: Bs {tasa.toFixed(2)}</span>
          </div>
    ...
    <div style={{ minHeight: "100vh", background: "#080818", fontFamily: "'DM Sans', sans-serif", color: "#e2e2f0" }}>
      ...
      <header style={{ padding: "12px 20px", borderBottom: "1px solid #1e1e3a", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(8,8,24,0.9)", backdropFilter: "blur(10px)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 9, background: role === "vendedor" ? "linear-gradient(135deg, #4338ca, #6366f1)" : "linear-gradient(135deg, #065f46, #10b981)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 13, fontWeight: 800, color: "#fff", fontFamily: "'JetBrains Mono', monospace" }}>V$</span>
          </div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 800 }}>Sistema de Ventas</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={() => setRole(null)} style={{ padding: "7px 14px", borderRadius: 8, border: "1px solid #1e1e3a", background: "#0e0e22", color: "#5a5a7a", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>← Salir</button>
        </div>
      </header>
      {/* Quick tasa edit */}
      {showHeaderTasa && (
      ...
      <main style={{ maxWidth: 860, margin: "0 auto", padding: "22px 16px" }}>
        {role === "vendedor" ? <Vendedor sales={sales} setSales={setSales} expenses={expenses} setExpenses={setExpenses} cambios={cambios} setCambios={setCambios} tasa={tasa} setTasa={setTasa} /> : <Admin sales={sales} expenses={expenses} setExpenses={setExpenses} cambios={cambios} setCambios={setCambios} tasa={tasa} />}
      </main>
    </div>
  );
}
