...
function Admin({ sales, expenses, setExpenses, cambios, setCambios, tasa, balanceInicial, saveBalanceInicial }) {
  ...
  const biPmov = Number(bi["Pago Móvil"]) || 0; const biPdv = Number(bi["Punto de Venta"]) || 0;
  const bancoIni = (biPmov + biPdv) / biTasa;
  const bancoIniBs = biPmov + biPdv;
  ...
  return (
    <div>
      ...
      {tab === "balance" && (() => {
        const methods = ["Pago Móvil", "Punto de Venta", "Efectivo Bs", "Efectivo $", "Binance", "Zelle"];
        return <>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Load config (balance inicial + tasa)
    getDocs(collection(db, "config")).then(snap => {
      ...
      const tasaDoc = snap.docs.find(d => d.id === "tasa");
      if (tasaDoc && tasaDoc.data().valor) {
        const t = Number(tasaDoc.data().valor);
  ...
  useEffect(() => { if (db && dbLoaded) setDoc(doc(db, "config", "tasa"), { valor: tasa }).catch(console.error); }, [tasa, dbLoaded]);
