...
function Admin({ sales, expenses, setExpenses, cambios, setCambios, tasa, balanceInicial, saveBalanceInicial }) {
  ...
  return (
    <div>
      ...
      {tab === "detalle" && <>
        <h3 style={{ margin: "0 0 6px 0", fontSize: 15, color: "#c0c0d8", fontWeight: 700 }}>Resumen Diario</h3>
        <div style={{ fontSize: 11, color: "#5a5a7a", marginBottom: 14 }}>👆 Toca un día para ver las ventas individuales</div>
            ...
            <tbody>
              {dates.map(d => {
                ...
                return <React.Fragment key={d}>
                  <tr onClick={() => setExpandedDay(isOpen ? null : d)} style={{ borderBottom: "1px solid #151530", cursor: "pointer", background: isOpen ? "#0f2e1e" : "transparent" }}>
                    <td style={{ padding: "8px 10px", color: "#c0c0d8", fontWeight: 700 }}>{isOpen ? "▼" : "▶"} {dayLabel}</td>
                  ...
                  {isOpen && <tr>
                    <td colSpan={7} style={{ padding: "10px 6px 16px", background: "#0a1a12" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {[...ds].sort((a, b) => (b.createdAt || b.id) > (a.createdAt || a.id) ? 1 : -1).map(s => {
                          const items = getItems(s);
                          const pagos = getPagos(s);
                          const total = saleTotal(s);
                          const ganancia = s.pago - saleCosto(s);
                          const time = s.createdAt ? new Date(s.createdAt).toLocaleTimeString("es-VE", { hour: "2-digit", minute: "2-digit", hour12: true }) : "";
                          const resumen = items.map(i => `${i.cantidad}× ${i.descripcion}`).join(", ");
                          return (
                            <div key={s.id} onClick={() => setDetailSale(s)} style={{ background: "#13132b", border: "1px solid #1e1e3a", borderRadius: 10, padding: "10px 14px", cursor: "pointer", transition: "all 0.15s" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                                    <span style={{ fontSize: 11, fontWeight: 800, color: "#c4b5fd", fontFamily: "'JetBrains Mono', monospace", background: "#1e1b4b", padding: "2px 7px", borderRadius: 5 }}>#{s.nota || "—"}</span>
                                    <span style={{ fontSize: 10, color: "#5a5a7a" }}>{time}</span>
                                  </div>
                                  <div style={{ fontSize: 12, color: "#ddddf0", marginBottom: 5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{resumen}</div>
                                  <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                                    {pagos.map((p, i) => <Badge key={i} method={p.metodo} small />)}
                                  </div>
                                </div>
                                <div style={{ textAlign: "right", flexShrink: 0 }}>
                                  <div style={{ fontSize: 14, fontWeight: 800, color: "#818cf8", fontFamily: "'JetBrains Mono', monospace" }}>{fmt(total)}</div>
                                  <div style={{ fontSize: 10, color: ganancia >= 0 ? "#34d399" : "#f87171", fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>+{fmt(ganancia)}</div>
                                </div>
                              </div>
                            </div>
                          );
        ...
        <Modal open={!!detailSale} onClose={() => setDetailSale(null)} title={detailSale ? `Detalle de Venta · #${detailSale.nota || "—"}` : ""} width={580}>
          {detailSale && (() => {
            const items = getItems(detailSale);
            const pagos = getPagos(detailSale);
            const total = saleTotal(detailSale);
            const costoTotal = saleCosto(detailSale);
            const ganancia = detailSale.pago - costoTotal;
            const margen = total > 0 ? (ganancia / total) * 100 : 0;
            const fecha = detailSale.createdAt
              ? new Date(detailSale.createdAt).toLocaleString("es-VE", { day: "2-digit", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: true })
              : detailSale.date;
            const tienenBs = pagos.some(p => USES_BS.has(p.metodo));
            return (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {/* Encabezado */}
                <div style={{ background: "#0e0e22", border: "1px solid #1e1e3a", borderRadius: 12, padding: "12px 14px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    <span style={{ fontSize: 10, color: "#5a5a7a", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>Fecha y hora</span>
                    {tienenBs && <span style={{ fontSize: 10, color: "#a38a2d", fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>Tasa: Bs {Number(detailSale.tasaBs || 0).toFixed(2)}</span>}
                  </div>
                  <div style={{ fontSize: 13, color: "#e2e2f0", fontWeight: 600, textTransform: "capitalize" }}>{fecha}</div>
                </div>

                {/* Items */}
                <div>
                  <div style={{ fontSize: 10, color: "#5a5a7a", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Productos / Servicios ({items.length})</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {items.map((it, i) => {
                      const subtotal = it.precioVenta * it.cantidad;
                      const costoLinea = it.costo * it.cantidad;
                      const ganLinea = subtotal - costoLinea;
                      return (
                        <div key={i} style={{ background: "#0e0e22", border: "1px solid #1e1e3a", borderRadius: 10, padding: "10px 12px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                                <TypeBadge tipo={it.tipo} />
                                <span style={{ fontSize: 11, color: "#5a5a7a", fontFamily: "'JetBrains Mono', monospace" }}>×{it.cantidad}</span>
                              </div>
                              <div style={{ fontSize: 13, color: "#e2e2f0", fontWeight: 600 }}>{it.descripcion}</div>
                            </div>
                            <div style={{ textAlign: "right", flexShrink: 0 }}>
                              <div style={{ fontSize: 14, fontWeight: 800, color: "#818cf8", fontFamily: "'JetBrains Mono', monospace" }}>{fmt(subtotal)}</div>
                            </div>
                          </div>
                          <div style={{ display: "flex", gap: 14, fontSize: 10, color: "#5a5a7a", borderTop: "1px dashed #1e1e3a", paddingTop: 6, fontFamily: "'JetBrains Mono', monospace" }}>
                            <span>P.U: <span style={{ color: "#93c5fd" }}>{fmt(it.precioVenta)}</span></span>
                            <span>Costo: <span style={{ color: "#f59e0b" }}>{fmt(costoLinea)}</span></span>
                            <span>Gan: <span style={{ color: ganLinea >= 0 ? "#34d399" : "#f87171", fontWeight: 700 }}>{fmt(ganLinea)}</span></span>
                          </div>
                        </div>
                      );
                    })}
                ...
                {/* Pagos */}
                <div>
                  <div style={{ fontSize: 10, color: "#5a5a7a", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Forma de pago ({pagos.length === 1 ? "Único" : "Mixto"})</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {pagos.map((p, i) => {
                      const enBs = USES_BS.has(p.metodo);
                      const montoBs = enBs ? p.monto * Number(detailSale.tasaBs || 0) : 0;
                      return (
                        <div key={i} style={{ background: "#0e0e22", border: "1px solid #1e1e3a", borderRadius: 10, padding: "9px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <Badge method={p.metodo} />
                          <div style={{ textAlign: "right" }}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: "#e2e2f0", fontFamily: "'JetBrains Mono', monospace" }}>{fmt(p.monto)}</div>
                            {enBs && montoBs > 0 && <div style={{ fontSize: 10, color: "#a38a2d", fontFamily: "'JetBrains Mono', monospace" }}>{fmtBs(montoBs)}</div>}
                          </div>
                        </div>
                      );
                    })}
                ...
                {/* Totales */}
                <div style={{ background: "linear-gradient(135deg, #0f2e1e, #13132b)", border: "1px solid #1a4a38", borderRadius: 14, padding: "14px 16px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, fontSize: 12 }}>
                    <span style={{ color: "#5a5a7a" }}>Total Venta</span>
                    <span style={{ color: "#818cf8", fontWeight: 800, fontFamily: "'JetBrains Mono', monospace", fontSize: 15 }}>{fmt(total)}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, fontSize: 12 }}>
                    <span style={{ color: "#5a5a7a" }}>Capital invertido</span>
                    <span style={{ color: "#f59e0b", fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>{fmt(costoTotal)}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 8, borderTop: "1px solid #1a4a38" }}>
                    <span style={{ color: "#6ee7b7", fontSize: 12, fontWeight: 700 }}>Ganancia</span>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ color: ganancia >= 0 ? "#34d399" : "#f87171", fontWeight: 800, fontFamily: "'JetBrains Mono', monospace", fontSize: 17 }}>{fmt(ganancia)}</div>
                      <div style={{ fontSize: 10, color: "#5a5a7a", fontFamily: "'JetBrains Mono', monospace" }}>Margen: {margen.toFixed(1)}%</div>
                    </div>
                  </div>
                </div>
              </div>
...
export default function App() {
  const [role, setRole] = useState(() => localStorage.getItem("vm_role") || null);
  ...
  const [tasa, setTasa] = useState(() => Number(localStorage.getItem("vm_tasa")) || 86.50);
  const [ready, setReady] = useState(false);
  const [tasaConfirmed, setTasaConfirmed] = useState(() => localStorage.getItem("vm_tasaDate") === getToday());
  ...
  useEffect(() => { if (role) localStorage.setItem("vm_role", role); else localStorage.removeItem("vm_role"); }, [role]);
  useEffect(() => { localStorage.setItem("vm_tasa", String(tasa)); }, [tasa]);
  useEffect(() => { if (db && dbLoaded && !isRemote.current) setDoc(doc(db, "config", "tasa"), { valor: tasa }).catch(console.error); }, [tasa, dbLoaded]);
  useEffect(() => { if (tasaConfirmed) localStorage.setItem("vm_tasaDate", getToday()); }, [tasaConfirmed]);
    ...
    <div style={{ minHeight: "100vh", background: "#080818", fontFamily: "'DM Sans', sans-serif", color: "#e2e2f0" }}>
      ...
      <header style={{ padding: "12px 20px", borderBottom: "1px solid #1e1e3a", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(8,8,24,0.9)", backdropFilter: "blur(10px)", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={() => { setRole(null); localStorage.removeItem("vm_role"); }} style={{ padding: "7px 14px", borderRadius: 8, border: "1px solid #1e1e3a", background: "#0e0e22", color: "#5a5a7a", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>← Salir</button>
        </div>
      </header>
      {/* Quick tasa edit */}
      {showHeaderTasa && (
